...
Let's for now ignore the `ItemListStateMachineFactory` and only focus on our new requirements: marking an `Item` as favorite (or unmark it) plus the communication with our backend server to store that information.
We could add this new requirements with  our DSL to `ItemListStateMachineFactory` somehow or we extract that into a small stand alone state machine.
...
Both `onActionStartStateMachine()` and `onEnterStartStateMachine()` also have effect counter parts: `onActionStartStateMachineEffect()` and `onEnterStartStateMachineEffect()`.
